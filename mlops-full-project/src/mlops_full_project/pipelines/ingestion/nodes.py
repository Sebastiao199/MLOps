...
def build_expectation_suite(expectation_suite_name: str, feature_group: str) -> ExpectationSuite:
    ...
    if feature_group == 'numerical_features':
        ...
        for i in ['Outpatient_visits', 'Emergency_visits', 'Inpatient_visits', 'Average_pulse_bpm', 'Length_of_stay', 
                'Number_lab_tests', 'Non_lab_procedures', 'Number_of_medications', 'Number_diagnoses', 'Number_generic_medications', 
                'Midpoint_Age', 'Total_visits', 'Serious_condition_visits', 'Number_encounters_total']:    
        ...
        # numerical features boundaries - none of them below zero (negative)
        #expectation_suite_hospital.add_expectation(
        #        ExpectationConfiguration(
    ...
    if feature_group == 'categorical_features':
        ...
        expectation_suite_hospital.add_expectation(
            ExpectationConfiguration(
                expectation_type="expect_column_distinct_values_to_be_in_set",
                kwargs={"column": "admission_type", "value_set": ['Emergency', 'Elective', 'Female', 'Urgent', 'Unknown', 'Not Available','Not Mapped', 'Trauma Center', 'Newborn']},
            ) 
        )
        expectation_suite_hospital.add_expectation(
            ExpectationConfiguration(
                expectation_type="expect_column_distinct_values_to_be_in_set",
                kwargs={"column": "discharge_disposition", "value_set": ['Discharged_to_home', 'Transferred_to_another_medical_facility', 'Unknown', 'Rehab', 'Expired', 'Other','Hospice']},
            )
        )
