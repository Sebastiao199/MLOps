...
def model_train(X_train: pd.DataFrame, 
    ...
    with mlflow.start_run(experiment_id=experiment_id, nested=True):
        ...
        logger.info(f"Production Model Accuracy: {accuracy:.4f}")
        logger.info(f"Production Model Precision: {precision:.4f}")
        logger.info(f"Production Model Recall: {recall:.4f}")
        logger.info(f"Production Model F1-score: {f1:.4f}")

        mlflow.log_metric("Production Model Accuracy", accuracy)
        mlflow.log_metric("Production Model Precision", precision)
        mlflow.log_metric("Production Model Recall", recall)
        mlflow.log_metric("Production Model F1_score", f1)

        # saving results in dict
        results_dict['classifier'] = classifier.__class__.__name__
        results_dict['train_score'] = f1_train
        results_dict['test_score'] = f1_test
        # logging in mlflow
    ...
    model_version = mlflow.register_model(f"runs:/{run_id}/model", "champion_model")
    mlflow.set_tag("model_description", f"New champion model: {model}")
