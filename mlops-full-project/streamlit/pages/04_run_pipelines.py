...
# any other custom pipeline for our Hospital case?
...
if choice != '':
    if stateful_button(f'**Run Pipeline**', key=f'**Run Pipeline**'):
        ...
        with redirect_stdout(io.StringIO()) as stdout_f, redirect_stderr(io.StringIO()) as stderr_f:
            try:
                good_process = subprocess.run(["kedro", "run", f"--pipeline={choice}"], cwd="../", capture_output=True, text=True)
        ...
        elif choice == "data_drift_pipeline":
            with open("..//data//08_reporting//data_drift_report.html",encoding="utf8") as f: 
                html_data = f.read()
            st.components.v1.html(html_data, scrolling=True, height=500)

            with open("..//data//08_reporting//univariate_nml.html",encoding="utf8") as f: 
                html_data = f.read()
            st.components.v1.html(html_data, scrolling=True, height=500)            
